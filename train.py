...
# data
if threads >= 0:
    cpu_config = tf1.ConfigProto(intra_op_parallelism_threads = threads//2,
                                inter_op_parallelism_threads = threads//2,
                                device_count = {'CPU': threads})
    sess = tf1.Session(config=cpu_config)
else:
    sess = tl.session()
...
# inputs
lr = tf1.placeholder(dtype=tf1.float32, shape=[])

xa = tr_data.batch_op[0]
a = tr_data.batch_op[1]
b = tf1.random_shuffle(a)
_a = (tf1.to_float(a) * 2 - 1) * thres_int
_b = (tf1.to_float(b) * 2 - 1) * thres_int
...
with tf1.control_dependencies([xb_]):
...
if mode == 'wgan':  # wgan-gp
    wd = tf1.reduce_mean(xa_logit_gan) - tf1.reduce_mean(xb__logit_gan)
    d_loss_gan = -wd
    gp = models.gradient_penalty(D, xa, xb_)
elif mode == 'lsgan':  # lsgan-gp
    xa_gan_loss = tf1.losses.mean_squared_error(tf1.ones_like(xa_logit_gan), xa_logit_gan)
    xb__gan_loss = tf1.losses.mean_squared_error(tf1.zeros_like(xb__logit_gan), xb__logit_gan)
    d_loss_gan = xa_gan_loss + xb__gan_loss
    gp = models.gradient_penalty(D, xa)
elif mode == 'dcgan':  # dcgan-gp
    xa_gan_loss = tf1.losses.sigmoid_cross_entropy(tf1.ones_like(xa_logit_gan), xa_logit_gan)
    xb__gan_loss = tf1.losses.sigmoid_cross_entropy(tf1.zeros_like(xb__logit_gan), xb__logit_gan)
    d_loss_gan = xa_gan_loss + xb__gan_loss
    gp = models.gradient_penalty(D, xa)

xa_loss_att = tf1.losses.sigmoid_cross_entropy(a, xa_logit_att)

d_loss = d_loss_gan + gp * 10.0 + xa_loss_att

# generator losses
if mode == 'wgan':
    xb__loss_gan = -tf1.reduce_mean(xb__logit_gan)
elif mode == 'lsgan':
    xb__loss_gan = tf1.losses.mean_squared_error(tf1.ones_like(xb__logit_gan), xb__logit_gan)
elif mode == 'dcgan':
    xb__loss_gan = tf1.losses.sigmoid_cross_entropy(tf1.ones_like(xb__logit_gan), xb__logit_gan)

xb__loss_att = tf1.losses.sigmoid_cross_entropy(b, xb__logit_att)
xa__loss_rec = tf1.losses.absolute_difference(xa, xa_)

g_loss = xb__loss_gan + xb__loss_att * 10.0 + xa__loss_rec * rec_loss_weight

# optim
d_var = tl.trainable_variables('D')
d_step = tf1.train.AdamOptimizer(lr, beta1=0.5).minimize(d_loss, var_list=d_var)

g_var = tl.trainable_variables('G')
g_step = tf1.train.AdamOptimizer(lr, beta1=0.5).minimize(g_loss, var_list=g_var)
...
if show_weights:
    d_histogram = tf1.summary.merge([tf1.summary.histogram(
        name=i.name,
        values=i
    ) for i in tf1.get_collection(tf1.GraphKeys.MODEL_VARIABLES, 'D')])
    
    genc_histogram = tf1.summary.merge([tf1.summary.histogram(
        name=i.name,
        values=i
    ) for i in tf1.get_collection(tf1.GraphKeys.MODEL_VARIABLES, 'Genc')])
        
    gdec_histogram = tf1.summary.merge([tf1.summary.histogram(
        name=i.name,
        values=i
    ) for i in tf1.get_collection(tf1.GraphKeys.MODEL_VARIABLES, 'Gdec')])
    
    gstu_histogram = tf1.summary.merge([tf1.summary.histogram(
        name=i.name,
        values=i
    ) for i in tf1.get_collection(tf1.GraphKeys.MODEL_VARIABLES, 'Gstu')])
    
    d_summary = tf1.summary.merge([d_summary, lr_summary, d_histogram])
    g_summary = tf1.summary.merge([g_summary, genc_histogram, gdec_histogram, gstu_histogram])
else:
    d_summary = tf1.summary.merge([d_summary, lr_summary])
...
# saver
saver = tf1.train.Saver(max_to_keep=num_ckpt)

# summary writer
summary_writer = tf1.summary.FileWriter('./output/%s/summaries' % experiment_name, sess.graph)
